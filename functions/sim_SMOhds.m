function [n_errors,hist_n_fails,sim_settings] = sim_SMOhds(n_sim,R,t_min,t_max)
% Simulate IMO helper data scheme with LDPC code of rate R using n_sim Monte
% carlo simulations. Evaluate FER for a range of t_min till t_max
% enrollment observations
sim_settings.n_sim = n_sim;
sim_settings.t_min = t_min; % number of enrollment observations
sim_settings.t_max = t_max; % max number of enrollments (is enrollments 
                            % + reconstructions)
...
hist_n_fails = zeros(1,t_max+1); % build histogram of nr. failures of an SRAM
for i = 1:n_sim
    ...
    M = xor(y,chIn);
    for n_enroll_init = 1:t_min-1
        y = int8(rand(length(SRAMPUF),1)<SRAMPUF);
        M = M+xor(y,chIn);
    end
    % prepare loop
    n_enroll = t_min; % number of enrollments used for M
    n_fails = 0;
    for n_rec = t_min:t_max % number of recovery trials (sometimes result in update)
        y = int8(rand(length(SRAMPUF),1)<SRAMPUF);
        %% now for the updated HD, M
        ...
        if tbErr>0
            n_errors(n_rec) = n_errors(n_rec)+1;
            n_fails = n_fails+1;
            continue % don't update HD M in this case
        end
        M = M+xor(y,chIn);
        n_enroll = n_enroll+1;
    end
    hist_n_fails(n_fails+1) = hist_n_fails(n_fails+1)+1;
...
if 0 % plot
    ...
    plot(t_enroll,FER,'-*');
    hold on;
    FER = n_errors_ref'/n_sim;
    plot(t_enroll,FER,'-*');
    legend('SMO scheme',sprintf('MO scheme for t=%d',t_min))
    set(gca, 'YScale', 'log')
    xlabel('n reconstructions');ylabel('FER')
    title(['SMO scheme, R = ',num2str(R)])
    grid on
pause(1)
end
